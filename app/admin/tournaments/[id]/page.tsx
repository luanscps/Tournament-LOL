...
export default async function AdminTournamentPage({
  ...
  const { count: totalTeams }    = await supabase.from("inscricoes").select("*", { count: "exact", head: true }).eq("tournament_id", id);
  const { count: pendingTeams }  = await supabase.from("inscricoes").select("*", { count: "exact", head: true }).eq("tournament_id", id).in("status", ["PENDING", "pending"]);
  const { count: approvedTeams } = await supabase.from("inscricoes").select("*", { count: "exact", head: true }).eq("tournament_id", id).in("status", ["APPROVED", "approved"]);
  const { count: checkedInTeams }= await supabase.from("inscricoes").select("*", { count: "exact", head: true }).eq("tournament_id", id).eq("checked_in", true);
  const { count: matchesCount }  = await supabase.from("matches").select("*", { count: "exact", head: true }).eq("tournament_id", id);
  const { count: stagesCount }   = await supabase.from("tournament_stages").select("*", { count: "exact", head: true }).eq("tournament_id", id);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <div className="mb-1">
            <Link href="/admin/tournaments" className="text-gray-400 hover:text-white text-sm">← Todos os Torneios</Link>
          </div>
        ...
        <div className="flex gap-2 flex-wrap justify-end items-center">
          ...
          <DeleteTournamentButton tournamentId={id} action={deleteTournament} />
        </div>
      </div>
      ...
      <div className="bg-[#0D1B2E] border border-[#1E3A5F] rounded-lg p-4">
        ...
        <GenerateBracketButton tournamentId={id} approvedTeams={approvedTeams ?? 0} currentStatus={t.status} />
      </div>
